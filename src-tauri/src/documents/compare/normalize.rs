...
mod tests {
    ...
    fn stop_words_are_filtered() {
        assert!(normalize("admin").is_none());
        assert!(normalize("Administrator").is_none());
        assert!(normalize("WPS Office").is_none());
        assert!(normalize("  ").is_none());
        assert!(normalize("高栋梁").is_some());
    }

    #[test]
    fn generic_templates_are_filtered() {
        assert!(normalize_template("Normal.dotm").is_none());
        assert!(normalize_template("C:\\Users\\x\\Normal.dotm").is_none());
        assert!(normalize_template("投标专用模板.dotx").is_some());
    }

    #[test]
    fn parses_common_timestamp_formats() {
        assert_eq!(parse_timestamp("1970-01-01T00:00:00Z"), Some(0));
        assert_eq!(parse_timestamp("2024-05-01T10:00:00Z"), Some(1_714_557_600));
        assert_eq!(
            parse_timestamp("2024-05-01 10:00:00"),
            parse_timestamp("2024-05-01T10:00:00Z")
        );
        assert_eq!(parse_timestamp("2024-05-01"), Some(1_714_521_600));
    ...
    #[test]
    fn timestamp_delta_supports_close_detection() {
        let a = parse_timestamp("2024-05-01T10:00:00Z").unwrap();
        let b = parse_timestamp("2024-05-01T10:05:00Z").unwrap();
        assert_eq!((a - b).abs(), 300);
    }
}
