...
export function selectGroups(result: CompareResult, state: CompareFilterState): AlignedGroup[] {
...
export function selectFindings(result: CompareResult, state: CompareFilterState): RiskFinding[] {
  ...
  return result.findings
    ...
    .filter(
      finding =>
        state.companyIds.length === 0 || finding.refs.some(ref => allowCompany.has(ref.companyId)),
    )
    .filter(
      finding => state.fieldKeys.length === 0 || finding.fieldKeys.some(key => allowField.has(key)),
    )
    .sort(
      (a, b) =>
        levelWeight(b.level) - levelWeight(a.level) || b.weight * b.score - a.weight * a.score,
    );
}
...
export function findGroupOfFinding(
  ...
  return result.groups.find(group =>
    group.cells.some(cell => cell.docId && docIds.has(cell.docId)),
  );
}
...
export function selectFindingClusters(
  ...
  for (const finding of visible) {
    ...
    for (const ref of finding.refs) {
      ...
      if (existingFile) {
        if (
          !existingFile.fields.some(
            field => field.key === ref.fieldKey && field.rawValue === ref.raw,
          )
        ) {
  ...
  for (const cluster of clusters) {
    ...
    if (cluster.ruleId === 'CLOSE_TIMESTAMP') {
      ...
      if (times.length) {
        suffix = formatTimeRange(new Date(Math.min(...times)), new Date(Math.max(...times)));
      }
    } else {
