...
import type { FileEntry, LoadedDocument, DocumentMetadata } from '@/types/document';
import type { DirectoryScanResult, DirectoryInfo } from '@/types/scan';
    ...
    addFiles: async (paths: string[]) => {
      ...
      try {
        ...
        const newFiles: FileEntry[] = paths.map(path => {
          ...
          const SUPPORTED_TYPES = ['docx', 'xlsx', 'pptx', 'pdf'] as const;
          const type = (SUPPORTED_TYPES as readonly string[]).includes(extension)
            ? (extension as FileEntry['type'])
            : 'unknown';
