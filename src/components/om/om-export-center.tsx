...
export const OmExportCenter: React.FC<OmExportCenterProps> = ({ fileIds = [], availableFields }) => {
  ...
  const selectedDocs = fileIds.length > 0
    ? documents.filter(d => fileIds.includes(d.id))
    : documents
  ...
  const handleExport = useCallback(async () => {
    ...
    try {
      ...
      const filePath = typeof outputDir === 'string'
      ...
      const normalizedData = selectedDocs.map(doc => {
        ...
        const fields = options.includeFields && options.includeFields.length > 0
          ...
          {exportResult && (
            <div className={`p-4 rounded-lg ${
              ...
              <div className="flex items-start gap-3">
                {exportResult.success ? (
                  <svg className="h-6 w-6 shrink-0 text-emerald-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" />
                  </svg>
                ) : (
                  <svg className="h-6 w-6 shrink-0 text-red-600 dark:text-red-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
