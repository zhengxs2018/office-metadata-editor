...
interface SaveOpsDeps {
  ...
  updateFileStatus: (
    documentId: string,
    patch: { status?: FileStatus; progressMessage?: string; error?: string },
  ) => void;
...
export function useMetadataSaveOps(deps: SaveOpsDeps): SaveOpsResult {
    ...
    async (documentId: string) => {
      ...
      try {
        ...
        setDocumentsById(prev => {
          ...
          return {
            ...prev,
            [documentId]: { ...current, originalMetadata: current.metadata, hasChanges: false },
          };
        });
  ...
  const batchClearAndSave = useCallback(async () => {
    ...
    try {
      ...
      setDocumentTaskRequestIds(prev => {
        const next = { ...prev };
        documents.forEach(item => {
          if (next[item.id] === requestId) delete next[item.id];
        });
        return next;
      });
    }
  ...
  const batchSaveAll = useCallback(async () => {
    ...
    setDocumentTaskRequestIds(prev => {
      const next = { ...prev };
      documents.forEach(item => {
        if (item.hasChanges) next[item.id] = requestId;
      });
      return next;
    });
